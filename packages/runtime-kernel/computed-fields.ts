...
import { evaluateExpression, validateExpressionBudget, type Expression } from '@/packages/runtime-kernel/expression';
import { executeQuery, stableJson } from '@/packages/runtime-kernel/query';
...
const DEFAULT_BUDGET = {
  ...
  maxExpressionRows: 256,
  maxExpressionRelations: 256,
  maxExpressionOperations: 512,
};
...
export function validateComputedFieldGraph(input: ComputedFieldGraphInput): void {
  ...
  for (const spec of input.specs) {
    ...
    validateExpressionBudget(spec.expression as Expression, {
      ...
      maxRows: budget.maxExpressionRows,
      maxRelations: budget.maxExpressionRelations,
      maxOperations: budget.maxExpressionOperations,
    });
  }
  ...
  const targets = [...new Set(input.collections.map((collection) => text(collection, 'computed_field_collection')))].sort();
  if (targets.length === 0 && input.specs.some((spec) => spec.collection === '*')) {
    targets.push('*');
  }
  for (const collection of targets) {
    validateScopedGraph(input.specs.filter((spec) => spec.collection === '*' || spec.collection === collection));
  }
}
...
function computedId(value: string): string {
