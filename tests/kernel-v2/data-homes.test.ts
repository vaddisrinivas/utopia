...
describe('data-home transport', () => {
  ...
  it('routes remote pull/push via HTTP transport', async () => {
    const fetcher = vi.fn(async (url, init) => {
      ...
      if (endpoint.endsWith('/data/tenant-a%3Afood%3Anotion/pull')) {
        expect(body).toEqual({ cursor: 'cursor-1', limit: 2 });
        return new Response(
          JSON.stringify({
            records: [{
      ...
      if (endpoint.endsWith('/data/tenant-a%3Afood%3Anotion/push')) {
    ...
    expect(fetcher.mock.calls[0][0]).toBe(`${baseUrl}/data/tenant-a%3Afood%3Anotion/pull`);
