...
import { checksum, contractChecksum, install, trustPublisher, restorePackage, uninstallPackage } from '@/src/kernel/registry';
...
describe('legacy-parity consolidation on v3 API', () => {
  ...
  it('enforces registry trust, checksum verification, and rollback restore', async () => {
    ...
    base.contractLock.checksum = await contractChecksum(base);
    ...
    await trustPublisher('kernel-legacy-v3', ed.etc.bytesToHex(publicKey), {
      capabilities: [...base.capabilities, 'permission:"camera"'],
    });
    await expect(install(entry)).resolves.toMatchObject({ id: base.id });

    const next = { ...base, version: '2.0.0' };
    next.contractLock = { ...base.contractLock, checksum: await contractChecksum({ ...next, contractLock: base.contractLock }) };
