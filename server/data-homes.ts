...
const scope = (config: Config, id: string) => `${config.tenantId}:${config.appId}:${id}`;
const unscoped = (config: Config, id: string) => id.startsWith(`${config.tenantId}:${config.appId}:`) ? id.split(':').slice(2).join(':') : undefined;

function notionHome(config: Extract<Config, { kind: 'notion' }>, token: string): Provider {
  const notion = new Notion({ auth: token });
  const notionMethods = notion as { dataSources?: { query?: (input: Record<string, unknown>) => Promise<{ results: unknown[]; has_more?: boolean; next_cursor?: string | null }> }; databases?: { query?: (input: Record<string, unknown>) => Promise<{ results: unknown[]; has_more?: boolean; next_cursor?: string | null }> } };
  ...
  const pages = async () => {
    ...
    do {
      const page = canQueryDataSource
        ? await notionMethods.dataSources!.query!({ data_source_id: config.resource, page_size: 100, result_type: 'page', start_cursor: cursor })
        : await notionMethods.databases!.query!({ database_id: config.resource, page_size: 100, start_cursor: cursor });
  ...
  const decode = (page: Awaited<ReturnType<typeof pages>>[number]): JsonRecord => {
    ...
    return {
      id: unscoped(config, text(props.UtopiaId?.rich_text) || '') ?? '',
  ...
  return {
    async pull(cursor) {
      const records = (await pages()).map(decode).filter((record) => record.id && record.updatedAt > (cursor ?? ''));
      return { records, cursor: records.reduce((latest, item) => item.updatedAt > latest ? item.updatedAt : latest, '') || undefined };
    },
    async push(records) {
      const existing = new Map((await pages()).map((page) => [decode(page), page.id] as const).filter(([record]) => record.id).map(([record, pageId]) => [record.id, { pageId, updatedAt: record.updatedAt }]));
      for (const record of records) {
        ...
        const properties = {
          ...
          UtopiaId: { rich_text: richText(scope(config, record.id)) },
        ...
        if (prior) await notion.pages.update({ page_id: prior.pageId, properties });
...
function sheetsHome(config: Extract<Config, { kind: 'google-sheets' }>, credentials: string): Provider {
  ...
  const decode = (row: unknown[]): JsonRecord => ({
    id: unscoped(config, String(row[0])) ?? '',
  ...
  return {
    async pull(cursor) {
      const records = (await rows()).slice(1).map(decode).filter((record) => record.id && record.updatedAt > (cursor ?? ''));
      return { records, cursor: records.reduce((latest, item) => item.updatedAt > latest ? item.updatedAt : latest, '') || undefined };
    },
    async push(records) {
      const current = await rows();
      const index = new Map(current.slice(1).map((row, offset) => [decode(row), offset + 2] as const).filter(([record]) => record.id).map(([record, row]) => [record.id, { row, updatedAt: record.updatedAt }]));
      const sheet = config.range.split('!')[0];
      for (const record of records) {
        const prior = index.get(record.id);
        if (prior && prior.updatedAt > record.updatedAt) continue;
        if (prior && record.values.deleted === true) {
          await sheets.spreadsheets.values.clear({ spreadsheetId: config.resource, range: `${sheet}!A${prior.row}:E${prior.row}` });
          continue;
        }
        const values = [[scope(config, record.id), record.collection, record.createdAt, record.updatedAt, JSON.stringify(record.values)]];
        if (prior) await sheets.spreadsheets.values.update({
          spreadsheetId: config.resource, range: `${sheet}!A${prior.row}:E${prior.row}`,
...
function postgresHome(config: Extract<Config, { kind: 'postgres' }>, connectionString: string): Provider {
  const db = drizzle(connectionString);
  const ready = db.execute(sql`create table if not exists utopia_records (
    scope text not null, id text not null, collection text not null, created_at timestamptz not null,
  ...
  return {
    async pull(cursor) {
      ...
      const result = await db.execute(sql`select id, collection, created_at, updated_at, values_json from utopia_records
        where scope = ${scope(config, '')} and updated_at > ${cursor || '1970-01-01T00:00:00.000Z'} order by updated_at asc limit 1000`);
    ...
    async push(records) {
      ...
      await db.transaction(async (tx) => {
        for (const record of records) {
          if (record.values.deleted === true) await tx.execute(sql`delete from utopia_records where scope=${scope(config, '')} and id=${record.id}`);
          else await tx.execute(sql`insert into utopia_records
          (scope, id, collection, created_at, updated_at, values_json) values
          (${scope(config, '')}, ${record.id}, ${record.collection}, ${record.createdAt}, ${record.updatedAt}, ${JSON.stringify(record.values)}::jsonb)
          on conflict (scope, id) do update set collection=excluded.collection, updated_at=excluded.updated_at, values_json=excluded.values_json
...
export function createDataHomes(value: unknown, env: NodeJS.ProcessEnv = process.env): Record<string, Provider> {
  return Object.fromEntries(z.array(Config).parse(value).map((config) => {
    ...
    const home = config.kind === 'postgres' ? postgresHome(config, secret)
      : config.kind === 'notion' ? notionHome(config, secret) : sheetsHome(config, secret);
    return [scope(config, config.id), home];
  }));
}
