import { Plus, RotateCcw, Search, SortAsc, SortDesc, Undo2 } from 'lucide-react-native';
...
import { Button, H2, Input, Label, Paragraph, ScrollView, Separator, Switch, Text, XStack, YStack } from 'tamagui';
...
function ListSurface({ component, collection, pkg, records, state, dispatch, query = '' }: Props & Runtime & { collection: string; records: JsonRecord[]; query?: string }) {
  ...
  }), [records, search, sortField, sortDirection, component.props?.limit]);
  ...
  return <Panel title={component.title}>
    ...
    <Separator />
    {rows.length ? rows.map((record) => <Button
      key={record.id}
      chromeless
      onPress={() => setForm(record)}
    ...
    </Button>) : <Paragraph color="$color10">No rows</Paragraph>}
  </Panel>;
}
