...
function Timer({ component }: Props) {
    ...
    <YStack gap="$4" style={{ alignItems: 'center' }}>
      {options.length > 1 ? <XStack gap="$2" flexWrap="wrap" accessibilityRole="radiogroup">{options.map((preset, index) =>
        <Button key={`${preset.label}-${preset.duration}`} size="$3" theme={selected === index ? 'green' : undefined}
          ...
          onPress={() => { setSelected(index); setRemaining(preset.duration); setRunning(false); }}>{preset.label}</Button>)}</XStack> : null}
      ...
      <XStack gap="$3">
        <Button circular size="$5" icon={running ? Pause : Play} onPress={() => setRunning((value) => !value)} aria-label={running ? 'Pause' : 'Start'} />
        <Button circular size="$5" icon={RotateCcw} onPress={() => { setRunning(false); setRemaining(duration); }} aria-label="Reset" />
      </XStack>
    </YStack>
  </Frame>;
}

function StepFlow({ component }: Props) {
  ...
  return <Frame title={component.title}>
    ...
    <Timer component={{ ...component, title: text(step.title), props: { durationSeconds: number(step.durationSeconds, 60) }, kind: 'widget' }} />
  </Frame>;
}
...
export function Widget({ appId = 'unknown', component, navigate = () => {} }: Props) {
  ...
  return useMemo(() => {
    ...
    switch (component.widget) {
      ...
      case 'durationTimer': return <Timer component={component} />;
      case 'stepFlow': return <StepFlow component={component} />;
