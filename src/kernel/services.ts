...
export async function syncDataHome(
  ...
  try {
    ...
    if (!secrets.UTOPIA_TENANT_ID) throw new Error('UTOPIA_TENANT_ID missing');
    provider = createDataHome(config as DataHomeConfig, secrets, storageLike, _baseUrl, {
      appId: pkg.id,
      tenantId: secrets.UTOPIA_TENANT_ID,
    });
  } catch (cause) {
