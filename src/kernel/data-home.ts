...
function remoteHome(config: RemoteDataHomeConfig, baseUrl: string): DataHomeTransport {
  const endpoint = `${baseUrl}/data/${encodeURIComponent(config.id)}`;
  ...
  return {
    async pull({ cursor, limit = 200 }) {
      const payload = { cursor, limit, kind: config.kind, secretRef: config.secretRef } as const;
    ...
    async push({ records }) {
      const result = await request<{ cursor?: string }>('push', { records, kind: config.kind, secretRef: config.secretRef });
...
export function createDataHome(
  ...
  return remoteHome(withSecretRef, readEndpoint(baseUrl, config));
}
