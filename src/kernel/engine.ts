...
const consume = (value: unknown, budget: Budget): void => {
  if (++budget.steps > MAX_WORK) throw new Error('expression_budget_exceeded');
  if (Array.isArray(value)) value.forEach((item) => consume(item, budget));
  else if (value && typeof value === 'object') Object.values(value as ObjectValue).forEach((item) => consume(item, budget));
};

const rows = (value: unknown): ObjectValue[] => {
  if (!Array.isArray(value) || value.length > MAX_WORK || value.some((item) => !item || typeof item !== 'object' || Array.isArray(item))) {
...
function evaluateDateDiff(spec: ObjectValue, context: unknown, budget: Budget): number {
...
function evaluate(expression: unknown, context: unknown, budget: Budget): unknown {
  if (++budget.steps > MAX_WORK) throw new Error('expression_budget_exceeded');
  ...
  if (entries.length !== 1) return Object.fromEntries(entries.map(([key, value]) => [key, evaluate(value, context, budget)]));
  const [operator, operand] = entries[0];
  if (operator === 'var') return read(context, String(operand));
  if (operator === 'date_diff') return evaluateDateDiff(object(operand, 'expression_date_diff_spec_invalid'), context, budget);
  if (operator === 'group_sum') {
    ...
    return rows(evaluate(spec.rows, context, budget))
  ...
  if (operator === 'allocate_weighted') {
    ...
    const source = rows(evaluate(spec.rows, context, budget));
    const total = Number(evaluate(spec.total, context, budget));
  ...
  if (operator === 'balance_transfers') {
    ...
    for (const row of rows(evaluate(spec.rows, context, budget))) {
  ...
  if (operator === 'recurrence_next' || operator === 'recurrence_expand') {
    ...
    const rule = recurrenceRule(evaluate(spec.schedule, context, budget), after);
    if (operator === 'recurrence_next') return rule.after(after, false)?.toISOString() ?? null;
    const limit = Math.min(100, Number(evaluate(spec.limit, context, budget) ?? 20));
    const until = spec.until ? new Date(String(evaluate(spec.until, context, budget))) : new Date(after.getTime() + 366 * 86_400_000);
...
export const evaluateExpression = (expression: unknown, context: unknown): unknown =>
  evaluate(expression, context, { steps: 0 });
...
export function computedRecords(pkg: AppPackage, state: AppState): JsonRecord[] {
  ...
  return state.records.map((record) => {
    ...
    const fields = pkg.computedFields.filter((item) => item.collection === record.collection);
    const pending = new Map(fields.map((field) => [field.id, field]));
    const ordered: typeof fields = [];
    while (pending.size) {
      const ready = [...pending.values()].filter((field) => (field.dependsOn ?? []).every((id) => !pending.has(id)));
      if (!ready.length) throw new Error(`computed_field_cycle:${[...pending.keys()].join(',')}`);
      ready.forEach((field) => { ordered.push(field); pending.delete(field.id); });
    }
...
export function reconcileTimer(timer: TimerSnapshot, now = Date.now()): TimerSnapshot {
  const previous = Date.parse(timer.updatedAt);
  if (!Number.isFinite(previous) || now < previous) return { ...timer, status: 'review', updatedAt: new Date(now).toISOString() };
  if (timer.status !== 'running') return timer;
  const elapsedMs = Math.min(timer.durationMs, timer.elapsedMs + now - previous);
  return { ...timer, elapsedMs, status: elapsedMs === timer.durationMs ? 'completed' : 'running', updatedAt: new Date(now).toISOString() };
}
