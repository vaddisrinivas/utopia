...
import { PackageTheme, usePackageTheme } from './theme';
...
export function PackageApp({ pkg, initialScreen }: { pkg: AppPackage; initialScreen?: string }) {
  ...
  const accent = identity?.accent ?? '#2F7448';
  const emoji = typeof identity?.emoji === 'string' ? identity.emoji : '◆';
  const ink = typeof identity?.ink === 'string' ? identity.ink : '#182019';
  ...
  return <PackageTheme identity={identity}><YStack accessibilityRole={Platform.OS === 'web' ? 'main' as never : undefined} style={{ flex: 1, backgroundColor: identity?.canvas ?? '#fff', ...layout(pkg.presentation.ui.layout, width, height, Platform.OS) }}>
