...
export function applyAction(state: AppState, action: AppAction): AppState {
  ...
  if (action.kind === 'undo') {
    ...
    return previous ? { records: previous, undo: state.undo?.slice(0, -1) } : state;
  }

  const commit = (records: JsonRecord[]): AppState => ({ records, undo: [...(state.undo ?? []).slice(-9), state.records] });
  ...
  if (action.kind === 'create' && action.collection) {
    ...
    const record: JsonRecord = existing
      ? { ...existing, collection: action.collection, updatedAt: now, values: { ...existing.values, ...action.values } }
      : { id, collection: action.collection, createdAt: now, updatedAt: now, values: action.values ?? {} };
    return commit(existing ? state.records.map((item) => item.id === id ? record : item) : [...state.records, record]);
  }
  ...
  if (action.kind === 'update' && action.recordId) {
    ...
    return commit(state.records.map((record) => record.id === action.recordId
      ? { ...target, updatedAt: now, values: { ...target.values, ...action.values } }
      : record));
  }

  if (action.kind === 'delete' && action.recordId) {
    const exists = state.records.some((record) => record.id === action.recordId);
    if (!exists) return state;
    return commit(state.records.filter((record) => record.id !== action.recordId));
  }

  if (action.kind === 'toggle' && action.recordId) {
    const field = String(action.payload?.field ?? 'completed');
    return commit(state.records.map((record) => record.id === action.recordId
      ? { ...record, updatedAt: now, values: { ...record.values, [field]: !record.values[field] } }
      : record));
  }
  ...
  if (action.kind === 'propose' && action.payload?.confirmed === true && action.operation) {
    ...
    if (operation === 'update') {
      ...
      const next = applyAction(state, { kind: 'update', recordId, values: action.values });
      return addReceipt(next, operation, next === state ? 'unavailable' : 'completed', recordId);
    }

    if (operation === 'delete') {
      if (!recordId) return addReceipt(state, operation, 'unavailable');
      const next = applyAction(state, { kind: 'delete', recordId });
      return addReceipt(next, operation, next === state ? 'unavailable' : 'completed', recordId);
    }
